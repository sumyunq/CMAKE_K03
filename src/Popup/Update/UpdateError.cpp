...
#include "ui_UpdateError.h"
#include <QGraphicsDropShadowEffect>

UpdateError::UpdateError(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::UpdateError)
{
    ui->setupUi(this);
    ...
    connect(ui->pBt_Retry,&QPushButton::clicked,this,&QDialog::accept);
...
//设置错误提示文本
void UpdateError::setTitle(int type)
{
    switch(type)
    {
    case 0:
        ui->lab_title->setText(tr("升级失败，请重试"));//上位机升级
        break;
    case 1:
        ui->lab_title->setText(tr("固件升级失败，请重试"));//固件升级
        break;
    case 2:
        ui->lab_title->setText(tr("固件回退失败，请重试"));//固件回退
        break;
    case 3:
        ui->lab_title->setText(tr("获取设备当前版本信息失败，请重试"));//获得设备当前版本失败
        break;
    default:
        break;
...
//根据主题设置样式
void UpdateError::setTheme_UpdateError(int idx)
{
    ...
    ui->lab_icon->setStyleSheet(
        QString("background:transparent;border-image: url(:/Skin/Images/Popup/updateError%1.png);")
            .arg(suffix)
        );
